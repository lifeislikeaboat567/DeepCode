...
class FileManagerTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            ...
            "Actions: read, write, list, tree, exists, share(send/upload/attach and *_file aliases)."
        )
    ...
    @staticmethod
    def _required_fields_for_action(action: str) -> tuple[str, ...]:
    ...
    async def run(self, action: str, **kwargs: Any) -> ToolResult:
        ...
        dispatch = {
            ...
            "upload": self._share,
            "attach": self._share,
            "send_file": self._share,
            "upload_file": self._share,
            "share_file": self._share,
            "attach_file": self._share,
        }
        ...
        missing_fields = [
            field
            for field in self._required_fields_for_action(action)
            if kwargs.get(field) in (None, "")
        ]
        if missing_fields:
            hint = ""
            if action == "write":
                hint = " Provide both 'path' and 'content'."
            elif action in {"share", "send", "upload", "attach", "send_file", "upload_file", "share_file", "attach_file"}:
                hint = " Provide 'path' to an existing file."
            return ToolResult.fail(
                self.name,
                f"Missing required argument(s) for action '{action}': {', '.join(missing_fields)}.{hint}",
            )
    ...
    async def _share(self, path: str, name: str | None = None, **_: Any) -> ToolResult:
        """Return a markdown attachment link for an existing file."""
        full_path = self._safe_path(path)
        if not full_path.exists():
            return ToolResult.fail(self.name, f"File not found: '{path}'")
        if not full_path.is_file():
        ...
        link_name = str(name or full_path.name or "attachment").strip() or "attachment"
        file_uri = full_path.as_uri()
        link = f"[{link_name}]({file_uri})"
        return ToolResult.ok(
            self.name,
            f"Attachment ready: {link}",
            path=str(full_path),
            file_uri=file_uri,
            markdown_link=link,
        )
